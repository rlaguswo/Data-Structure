...
/* Allocate and initialize dynamic array.
    ...
    pre:  none
    post: noneret:  a non-null pointer to a dynArr of cap capacityand 0 elements in it.
*/
...
/* Deallocate data array and the dynamic array structure. 
    param:      v           pointer to the dynamic array
    pre:  nonepost: the memory used by v->data is freed
...
/*    Get an element from the dynamic array from a specified position
    ...
    pre:  v is not nullpre:  v is not empty
...
/*    Returns boolean (encoded in an int) demonstrating whether or not the dynamic array stack has an item on it.
    ...
    post: noneret:  1 if empty, otherwise 0
*/
...
/*    Push an element onto the top of the stack
    ...
    pre:  v is not null
    post: size increases by 1if reached capacity, capacity is doubledval is on the top of the stack
*/
void pushDynArr(DynArr *v, TYPE val){
    if (v->size >= v->capacity)_dynArrSetCapacity(v,v->size*2);
    v->data[v->size] = val;
    v->size++;
}
...
/* Removes the element on top of the stack 
    ...
    pre:  v is not null
    pre:  v is not empty
    post: size is decremented by 1the top has been removed
*/
...
/*    Returns boolean (encoded as an int) demonstrating whether or notthe specified value is in the collection
    param:      v           pointer to the dynamic array
...
/*    Removes the first occurrence of the specified value from the collectionif it occurs
    param:      v           pointer to the dynamic array
